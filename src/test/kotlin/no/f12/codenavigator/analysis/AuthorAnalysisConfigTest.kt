...
class AuthorAnalysisConfigTest {
    ...
    fun `parses all properties from map`() {
        ...
        val config = AuthorAnalysisConfig.parse(props)
    ...
    fun `defaults after to approximately one year ago when absent`() {
        val config = AuthorAnalysisConfig.parse(emptyMap())
    ...
    fun `sets followRenames to false when no-follow present`() {
        val config = AuthorAnalysisConfig.parse(mapOf("no-follow" to null))
    ...
    fun `parses LLM format`() {
        val config = AuthorAnalysisConfig.parse(mapOf("llm" to "true"))
